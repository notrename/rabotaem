...
class TestAPI:
    ...
    def get_token(self):
        ...
        return response_data['token']  # Убедитесь, что ключ 'token' соответствует вашему ответу
    ...
    def create_booking(self):
        ...
        assert response.status_code == 200, f"Ожидался статус 200, но получен {response.status_code}"
        response_data = response.json()
        self.booking_id = response_data['bookingid']
    ...
    def test_ping_api(self):
        ...
        with allure.step('Проверка тела ответа'):
            assert response.text == "Created", f"Ожидался ответ 'Created', но получен '{response.text}'"
    ...
    def test_delete_booking(self):
        ...
        get_response = Api.get(url=f"{self.base_url}/{self.booking_id}")
        assert get_response.status_code == 404, f"Ожидался статус 404, но получен {get_response.status_code}. Ответ: {get_response.text}"
